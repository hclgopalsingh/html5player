...
export class Template1Component implements OnInit,OnDestroy ,AfterViewChecked,AfterViewInit{
    ...
    doRandomize(array) {
        ...
        while (0 !== currentIndex) {
            ...
            currentIndex -= 1;
            const img_hover1 = array[currentIndex].optBgHover;
            const text1copy = array[currentIndex].optBgOriginal;
            const optionBg1 = array[currentIndex].optBg;

            const img_hover2 = array[randomIndex].optBgHover;
            const text2copy = array[randomIndex].optBgOriginal;
    ...
    /******On Hover option ********/
    onHoverOptions(index) {
        this.optionRef.nativeElement.children[index].classList.add("optionHover");
        const speakerEle = document.getElementsByClassName("speakerBtn")[0].children[1] as HTMLAudioElement;
    ...
    selectOptMatra(opt, idx) {
        ...
        const optionURL = opt.url;
        const matraName = optionURL.split('.')[0].split('/').pop();
        this.saveOpt.classList.add('matra_' + matraName);
        const showMatra = opt.location == "content" ? this.containgFolderPath + "/" + opt.url : this.assetsPath + "/" + opt.url;
            ...
            this.rightSelectTimer = setTimeout(() => {
                ...
                if (this.rightFeedback && this.rightFeedback.nativeElement) {
                    ...
                    this.clapTimer = setTimeout(() => {
                        this.clapSound.nativeElement.pause();
                        this.clapSound.nativeElement.currentTime = 0;
                        const ansPopup: HTMLElement = this.ansPopup.nativeElement as HTMLElement
