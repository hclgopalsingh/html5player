...
@Component({
    ...
    templateUrl: '../view/layout/template3.component.html',
    styleUrls: ['../view/css/template3.component.css', '../view/css/bootstrap.min.css'],
			...
			// Analytics called for attempt counter & first option is clicked
				...
				if(!this.narrator_voice.nativeElement.paused){
					...
					if(option.id==this.feedback.correct_ans_index){
						...
						let audio;
						if(this.feedback.write_ans_sound.path.location=="content"){
							audio = this.assetspath;
						}else{
							audio = this.assetsfolderlocation;
						}
						this.playSound(audio +"/" +this.feedback.write_ans_sound.path.url);
						//this.isLastQues = this.appModel.isLastSection;
